...
const screenshot = {
  ...
  cache: {
    cacheControl: { // these cache control directives are sent in the cache-control header of the response to screenshot api
      maxAge: {
        $filter: 'env',
        local: 1,
        $default: 60
      },
      sMaxAge: { // this is just for the CDN, it will mark screenshots as stale after 24 hours. we use keycdn plugin to purge it on item.update
        $filter: 'env',
        local: 1,
        $default: 60 * 60 * 24
      },
      staleWhileRevalidate: { // this is just for the CDN, it will serve maximum 7 days old content while revalidating in the background
        $filter: 'env',
        local: 1,
        $default: 60 * 60 * 24 * 7
      },
      staleIfError: {  // this is just for the CDN, it will serve maximum 7 days old content if the backend is responding with an error
        $filter: 'env',
        local: 1,
        $default: 60 * 60 * 24 * 7
      }
